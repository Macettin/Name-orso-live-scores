...
alter table public.players add column if not exists yellow_cards integer not null default 0;
alter table public.players add column if not exists red_cards integer not null default 0;

drop policy if exists "admins write match stats" on public.match_stats;
drop policy if exists "scorers write match stats" on public.match_stats;
...
begin
  ...
  if p_stat_value is null or p_stat_value <= 0 then
    raise exception 'Stat value must be positive.';
  end if;
  ...
  if p_stat_key in ('points', 'goals') then
    ...
    set home_score = home_score + case when player_team_id = match_home_team_id then p_stat_value else 0 end,
        away_score = away_score + case when player_team_id = match_away_team_id then p_stat_value else 0 end
    where id = p_match_id;
  end if;
end;
$$;
